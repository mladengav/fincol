...
class AggregationUpdater:
    ...
    def update_aggregations(self, fincol_io: IFincolIo) -> None:
        ...
        self._update_dividends_by_year(fincol_io)
        last_decrease_by_ticker = self._update_last_dividend_decrease(fincol_io)
        self._update_years_since_dividend_decrease(
            fincol_io, last_decrease_by_ticker
        )
        self._update_years_consecutive_dividend_increase(fincol_io)
    ...
    def _update_years_since_dividend_decrease(
        ...
        fincol_io.write_years_since_dividend_decrease(years_since_by_ticker)

        print(f"Loaded {len(years_since_by_ticker)} ticker(s) from {fincol_io!r}")
        for sym in sorted(years_since_by_ticker):
            print(
                f"  Years since dividend decrease: {sym} = {years_since_by_ticker[sym]}"
            )

        print(f"Wrote years since dividend decrease to {fincol_io!r}")
    ...
    def _update_years_consecutive_dividend_increase(
        ...
        fincol_io.write_years_consecutive_dividend_increase(
        ...
        print(f"Wrote years consecutive dividend increase to {fincol_io!r}")
